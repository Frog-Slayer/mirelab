...
export default function PostPage() {
  ...
  return (
    <article className="mx-auto max-w-3xl">
      ...
      {post.work ? (
        <header className="mt-8 flex gap-6 rounded-xl border border-neutral-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="w-36 flex-none sm:w-44"><Cover work={post.work} size="lg" /></div>
          <div className="min-w-0 flex-1">
            <div className="flex items-start gap-3">
              <div>
                ...
                <h1 className="mt-2 text-3xl font-semibold tracking-[-0.03em]">{post.work.title}</h1>
              </div>
              {own && shelfEditHref && (
                <Link to={shelfEditHref} className="ml-auto text-sm text-neutral-500 hover:text-neutral-900">
                  책장에서 수정
                </Link>
              )}
            </div>
            <p className="mt-2 text-sm text-neutral-500">{post.work.author} · {post.work.year}</p>
            {post.work.description && (
              <p className="mt-4 text-sm leading-relaxed text-neutral-600">{post.work.description}</p>
            )}
            <p className="mt-4 text-xs text-neutral-400">{post.author.name}의 기록</p>
          </div>
        </header>
      ) : (
      ...
      {post.work && (
        <div className="mt-10 border-b border-neutral-200 pb-5">
          <h2 className="text-3xl font-semibold tracking-[-0.03em]">{post.title}</h2>
        </div>
      )}
