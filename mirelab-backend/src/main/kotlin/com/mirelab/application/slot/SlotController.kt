...
class SlotController(private val slotService: SlotService) {
    ...
    fun getSlots(@PathVariable workId: UUID): ResponseEntity<WorkSlotsResponse> =
        slotService.getWorkSlots(workId)?.let { ResponseEntity.ok(it) } ?: ResponseEntity.notFound().build()
